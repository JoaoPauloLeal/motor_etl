...
where oma.i_entidades = {{entidade}} and i_obras not in (select substring(i_chave_dsk2,0,charindex('/',i_chave_dsk2)-1) from bethadba.controle_migracao_registro cmr where tipo_registro = 'iniciar-obras' and i_chave_dsk1 = {{entidade}})
GROUP BY i_obras, i_entidades, id_obra