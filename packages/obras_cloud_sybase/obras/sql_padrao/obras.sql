SELECT
 ...
 LEFT JOIN bethadba.controle_migracao_registro B
    ON (A.categoria = B.i_chave_dsk2 and B.tipo_registro = 'categoria-obra' and b.i_chave_dsk1 = {{entidade}})
 LEFT JOIN bethadba.controle_migracao_registro C
 	ON (A.i_tipo_obras = C.i_chave_dsk2 and C.tipo_registro = 'tipos-obra' and c.i_chave_dsk1 = {{entidade}})
 INNER JOIN bethadba.controle_migracao_registro D
 	ON (D.i_chave_dsk3 = 'Outras' and D.tipo_registro = 'tipos-obra' and d.i_chave_dsk1 = {{entidade}})
   WHERE i_entidades = {{entidade}} and i_obras not in (select i_chave_dsk2 from bethadba.controle_migracao_registro cmr where tipo_registro = 'obras' and i_chave_dsk1 = {{entidade}})
