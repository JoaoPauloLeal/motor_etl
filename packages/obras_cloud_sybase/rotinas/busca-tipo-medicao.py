...
def iniciar_processo_envio(params_exec, *args, **kwargs):
    try:
        for contador, item in enumerate(
            ...
            descricao_tipo_reg,id_gerado,i_chave_dsk1,i_chave_dsk2,i_chave_dsk3) select 1,'tipo-medicao',
            '{hash}','Cadastro de Tipo de Medição','{item['id']}','{params_exec['entidade']}','{contador}','{str(item['descricao']).replace("'",'.')}' where not 
            EXISTS (SELECT 1 from bethadba.controle_migracao_registro where id_gerado = '{item['id']}' and i_chave_dsk1 = '{params_exec['entidade']}' and tipo_registro = 'tipo-medicao')""",
