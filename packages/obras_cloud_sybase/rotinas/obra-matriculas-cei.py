...
def iniciar_envio(params_exec, dados_assunto):
    ...
    for i in dict_dados:
        ...
        if not db.checa_existe_registro(params_exec, tipo_registro, str(i['i_obras'] + '/' + i['i_matricula_cei']), None, None):
            id_registro, mensagem_erro = cloud.envia_registro(f'https://obras.betha.cloud/obras/api/obras/{i["id_obra"]}/matriculasobras', json_envio, params_exec, False)
