...
import json
import packages.ipm_cloud_postgresql.frotas.rotinas_envio.funcionario as funcionario
import logging
...
sistema = 306
limite_lote = 50
url = "https://frotas.betha.cloud/frotas-services/api/conversoes/lotes/funcionarios"
...
def coletar_dados(params_exec, ano):
    ...
    try:
        ...
        df = pgcnn.exec_sql(query, index_col='codigo')
...
def iniciar_envio(params_exec, dados, metodo, *args, **kwargs):
    ...
    #
    for item in dados:
        hash_chaves = model.gerar_hash_chaves(sistema, tipo_registro, item["nome"], item["cpfcnpj"])
        # print("EMPACOU LOOP 1")
        dict_dados = {
            "hash": hash_chaves,
            "conteudo": {
                "nome": item["nome"],
                "tipo": item["tipopessoa"],
                "cpf": item["cpfcnpj"],
                "dataNascimento": item["datanascimento"],
                "dataAdmissao": item["admissao"],
            }
        }
        ...
        lista_controle_migracao.append({
            ...
            'descricao_tipo_registro': 'Cadastro de Funcionario',
            'id_gerado': None,
            'i_chave_dsk1': item["nome"],
            'i_chave_dsk2': item["cpfcnpj"]
        })
