...
def iniciar_envio(params_exec, dados, metodo, *args, **kwargs):
    ...
    for item in dados:
        ...
        dict_dados = {
            ...
            "conteudo": {
                "veiculoEquipamento": {
                    "id": int(item["veiculoequipamento"])
                },
                "motorista": {
                    "id": int(item["motorista"])
                },
                "fornecedor": {
                    "id": int(item["fornecedor"])
                },
                "organograma": {
                ...
                "dataLancamentoDespesa": item["datalancamentodespesa"],
                "numeroLancamentoDespesa": int(item["numerolancamentodespesa"]),
                "numeroDocumento": int(item["numerodocumento"]),
                "origem": int(item["origem"]),
                "controlaKmHoraMilha": item["controlakmhoramilha"],
                "observacao": item["observacao"],
                "origem": item["origem"],
                "situacaoCadastral": item["situacaocadastral"],
                "numeroDocto": int(item["numerodocto"]),
                "situacaoCadastral": item["situacaocadastral"],
                "quantidadeItensLancamentoDespesa": int(item["quantidadeitenslancamentodespesa"]),
                "valorTotalItensLancamentoDespesa": int(item["valorTotalitenslancamentodespesa"])
            },
            "context": {
