...
tipo_registro = 'correcao_organogramas_usuario_processo'
sistema = 304
limite_lote = 100
...
def iniciar_envio(params_exec, dados, metodo, *args, **kwargs):
    ...
    for item in dados:
        dict_dados = {
            "idIntegracao": str(item["id_organograma_usuario"]),
            "idGerado": int(item["id_organograma_usuario"]),
            "conteudo": {
                "id": int(item["id_organograma_usuario"]),
                "organograma": {
                    "id": int(item["id_organograma_andamento"])
                }
            }
        }
        contador += 1
        ...
        lista_controle_migracao.append({
            ...
            'hash_chave_dsk': str(item["id_organograma_usuario"]),
            'descricao_tipo_registro': 'Correcao organogramas',
            'id_gerado': None,
            'i_chave_dsk1': item["id_organograma_usuario"]
        })
