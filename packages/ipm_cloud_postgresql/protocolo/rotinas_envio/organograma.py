...
import packages.ipm_cloud_postgresql.protocolo.rotinas_envio.buscaOrganograma as organogramaBusca
...
def iniciar_processo_envio(params_exec, ano, *args, **kwargs):
    organogramaBusca.iniciar_processo_busca(params_exec)
...
def iniciar_envio(params_exec, dados, metodo, *args, **kwargs):
    ...
    contador = 0
    for item in dados:
        hash_chaves = model.gerar_hash_chaves(sistema, tipo_registro, item["organogramaconfig"], item["key1"])
        dict_dados = {
            "idIntegracao": hash_chaves,
            "conteudo": {
                "organogramaConfig": {
                        "id": item["organogramaconfig"]
                },
                "nome": item["nome"],
                "nivel": item["nivel"],
                "ativo": item["ativo"],
                "protocolizacao": item["protocolizacao"],
                "atribuicao": item["atribuicao"],
                "mascara": item["mascara"],
                "mascaraCompleta": item["mascaraCompleta"],
                "mascaraFormatada": item["mascaraFormatada"],
                "usuarios": [
                    {
                        "id": "lisandrobv"
                    }
        ...
        lista_controle_migracao.append({
            ...
            'id_gerado': None,
            'i_chave_dsk1': item["organogramaconfig"],
