...
class PostgreSQLConnection:
    ...
    def configura_banco(self):
        ...
        try:
            path = f"sistema_origem/{settings.BASE_ORIGEM}/{settings.SISTEMA_ORIGEM}/sql_padrao/config.sql_padrao"
...
def new_connection(dbname):
    ...
    try:
        conn_db: object = connect(f'DSN={dbname};UID=desbth;PWD=HsV0zk3yXYACypmaRsZ1m5POCC4v2y',
                                  autocommit=True,
...
def atualiza_tabela_controle_lote(**kwargs):
    ...
    try:
        pgcnn = PostgreSQLConnection()
        cursor = pgcnn.conn.cursor()
        result = cursor.execute(kwargs.get('sql_padrao'))
        pgcnn.conn.commit()
