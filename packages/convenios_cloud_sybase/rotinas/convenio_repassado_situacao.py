...
sistema = 1
tipo_registro = 'convenio_repassado_situacao'
...
def iniciar_envio(params_exec, dados_assunto):
    ...
    for item in dados_assunto:
        url = f"https://convenios.cloud.betha.com.br/convenios/api/convenios-repassados/{item['id_gerado_convenio']}/situacoes"
        dict_dado = {
            "tipoSituacao": {
            ...
            "convenio": {
                "id": int(item['id_gerado_convenio'])
            }
        }
        ...
        if id_gerado is not None:
            registro = [
                ...
                tipo_registro,
                'Cadastro de convenio repassado situacao',
                str(id_gerado),
                str(params_exec['id_entidade']),
                str(item['id_gerado_convenio']),
