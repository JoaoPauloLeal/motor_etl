...
sistema = 1
tipo_registro = 'convenio_recebido_situacao'
...
def iniciar_envio(params_exec, dados_assunto):
    ...
    for item in dados_assunto:
        url = f"https://convenios.cloud.betha.com.br/convenios/api/convenios-recebidos/{item['id_gerado_convenio']}/situacoes"
        dict_dado = {
            "tipoSituacao": {
        ...
        if id_gerado is not None:
            registro = [
                ...
                tipo_registro,
                'Cadastro de convenio recebido situacao',
                str(id_gerado),
