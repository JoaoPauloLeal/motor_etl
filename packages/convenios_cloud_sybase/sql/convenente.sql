select DISTINCT tab.cnpj_beneficiario, tab.beneficiario from (
SELECT   sapo.convenios.i_entidades ,
         ...
         sapo.convenios.num_transf_tce,
			...
			--(select first isnull(cgc,cpf) from sapo.credores where SOUNDEX(beneficiario) like SOUNDEX(nome) and (cpf is not null or cgc is not null)) as cnpj_beneficiario,
			protege_convenio = 0,
			regerar_lanctos = 'N',
...
where tab.cnpj_beneficiario is not null