...
def iniciar_processo_busca(params_exec, ano, *args, **kwargs):
    ...
    for x in db.consulta_sql(sql="select 1 as tt, * from bethadba.tempProc where chave_dsk2 in (2019,2020,2021)", index_col="tt", params_exec=params_exec):
        ...
        for y in cloud.buscaFonte(fields='id', criterio=filtro,url=url,token=params_exec['token']):
            ...
            ur = 'https://contratos.betha.cloud/contratos/dados/api/apostilamentos'
            for z in cloud.buscaFonte(url=ur, fields='id', criterio=fil, token=params_exec['token']):
                urldelete = f"https://contratos.betha.cloud/contratos/api/contratacoes/{y['id']}/apostilamentos/{z['id']}"
                print(urldelete)
