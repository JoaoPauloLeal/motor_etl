...
def iniciar_processo_busca(params_exec, *args, **kwargs):
    ...
    # db.execute_sql(f"insert into sapo.conexoes_usr (i_conexoes,usuario,ano_exerc,i_entidades,i_pontos,i_mov_diaria,
    # i_sistema,i_plano_contas,eh_4320,ctrl_trigger) on existing skip select @@spid as i_conexoes, current user
    # usuario, 2021 exerc ,{params_exec['entidade']} entid, null i_pontos,null i_mov_diaria, 1 sistema, 4,3,
    # 0 from dummy", conn)
    ...
    for x in db.consulta_sql(db.get_consulta(params_exec, 'valida-endereco-tab-temp6.sql'), conn=conn,
                             params_exec=params_exec, index_col='tipo_registro'):
        mostra = False
        arquivo = open('packages/tools/contabil/relatorios/endereco.txt', 'a')
        resultado = f"Registro {x['registro']} :"
        if str(x['registro'])[0:9] == 'do Credor':
            credor = str(x['registro'][10:])
            if x['cep_rua'] is not None:
                if len(x['cep_rua']) != 8:
                    resultado += ', cep com tamanho diferente de 8'
                    mostra = True
            if x['cep_rua'] == None:
                resultado += ', cep'
                mostra = True
                ...
                if correcao.upper() in ('SYSIMYES'):
                    db.execute_sql(f"""update bethadba.pessoas_enderecos set cep = '{cep}'  where
                                    ...
                                    where wh.i_credores = {credor} and wh.i_entidades = {params_exec['entidade']})""",
                    ...
                    db.execute_sql(f"""update bethadba.pessoas_enderecos a set nome_rua = (select first wh.endereco from sapo.credores wh where wh.i_credores = 101280) 
                                    where a.i_pessoas = (select first i_pessoas from sapo.credores wh where wh.i_credores = {credor})""",
                                   conn=conn, params_exec=params_exec)
                    print('Corrigido..')
            # if x['numero'] == None:
            #     resultado += ', numero'
