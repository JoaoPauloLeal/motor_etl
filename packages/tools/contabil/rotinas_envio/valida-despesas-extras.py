...
def iniciar_processo_busca(params_exec, *args, **kwargs):
    corrigir = str(input('Realizar correção, caso contrario será apenas demonstrado os erros ?'))
    ...
    for x in db.consulta_sql(db.get_consulta(params_exec,'valida-despesas-extras.sql'), params_exec=params_exec, index_col='i_entidades'):
    ...
    for x in db.consulta_sql(db.get_consulta(params_exec, 'despesas-extras-formapgto.sql'), params_exec=params_exec, index_col='i_entidades'):
        print(f"Despesa Extra {x['i_despexs']} com forma de Pagamento 'C' ")
        if corrigir.upper() in ('SYSIMYES'):
            conn = db.conectar(params_exec)
            db.execute_sql(f"CALL bethadba.pg_habilitartriggers('off');", conn=conn, params_exec=params_exec)
            db.execute_sql(f"update sapo.forma_pgto set formapg = 'D' where ano_exerc = {x['ano_exerc']} and i_entidades = {params_exec['entidade']} and i_despexs is not null and formapg != 'D' and i_despexs = {x['i_despexs']}", conn=conn, params_exec=params_exec)
