...
def iniciar_processo_busca(params_exec, *args, **kwargs):
    ...
    data = db.consulta_sql(sql=sql, params_exec=params_exec)
    total = len(data)
    for x in data:
        print(x)
        ...
        for retorna in cloud.buscaFonte(url=url, criterio=criterio, fields=fields, token=params_exec['token'], params_exec= params_exec):
            conta += 1
            qslq = f"update bethadba.controle_migracao_registro set id_gerado = {retorna.get('id')} where hash_chave_dsk = '{hash}'"
            print(qslq)
            db.execute_sql(sql=qslq, params_exec=params_exec)
