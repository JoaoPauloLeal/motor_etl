...
def iniciar_processo_busca(params_exec, *args, **kwargs):
    ...
    for x in db.consulta_sql(sql, index_col='sistema', params_exec=params_exec):
        ...
        if float(valorArrecDsk) != round(valorArrecCloud, 2):
            print(
                f'Valor de arrecadação: Cloud: R$ {round(valorArrecCloud, 2)} Desktop: R$ {float(valorArrecDsk)} - Inconsistente')
        else:
    ...
    for x in db.consulta_sql(sql, index_col='sistema', params_exec=params_exec):
        ...
        if float(valorArrecDsk) != round(valorArrecCloud, 2):
            print(
                f'Valor de D.E: Cloud: R$ {round(valorArrecCloud, 2)} Desktop: R$ {float(valorArrecDsk)} - Inconsistente')
        else:
    ...
    for x in db.consulta_sql(sql, index_col='sistema', params_exec=params_exec):
        ...
        if float(valorArrecDsk) != round(valorArrecCloud, 2):
            print(
                f'Valor de Empenhos: Cloud: R$ {round(valorArrecCloud, 2)} Desktop: R$ {float(valorArrecDsk)} - Inconsistente')
        else:
    ...
    for x in db.consulta_sql(sql, index_col='sistema', params_exec=params_exec):
        ...
        if float(valorLiqDsk) != round(valorLiqCloud, 2):
            print(
                f'Valor de Liquidacoes: Cloud: R$ {round(valorLiqCloud, 2)} Desktop: R$ {float(valorLiqDsk)} - Inconsistente')
        else:
    ...
    for x in db.consulta_sql(sql, params_exec, index_col='sistema'):
        if x['valor'] is not None:
            valorLiqDsk = float(x['valor'])
            valorLiqCloud = 0
            ...
            if float(valorLiqDsk) != round(valorLiqCloud, 2):
                print(
                    f'Valor de Anl Liquidacoes: Cloud: R$ {round(valorLiqCloud, 2)} Desktop: R$ {float(valorLiqDsk)} - Inconsistente')
            else:
                print(
                    f'Valor de Anl Liquidacoes: Cloud R$ {round(valorLiqCloud, 2)} Desktop R$ {float(valorLiqDsk)} - OK')
        else:
            print('Valor de Anl Liquidacoes: Cloud R$ 0,00 Desktop R$ 0,00')
    print('Fim Execução!')
