...
def iniciar_processo_busca(params_exec, *args, **kwargs):
    ...
    db.execute_sql(db.get_consulta(params_exec, 'valida-endereco-tab-temp1.sql'), params_exec=params_exec, conn=conn)
    db.execute_sql(db.get_consulta(params_exec, 'valida-endereco-tab-temp2.sql'), params_exec=params_exec, conn=conn)
    db.execute_sql(db.get_consulta(params_exec, 'valida-endereco-tab-temp4.sql'), params_exec=params_exec, conn=conn)
    db.execute_sql(db.get_consulta(params_exec, 'valida-responsaveis-temp1.sql'), params_exec=params_exec, conn=conn)
    db.execute_sql(db.get_consulta(params_exec, 'valida-responsaveis-temp2.sql'), params_exec=params_exec, conn=conn)
    db.execute_sql(db.get_consulta(params_exec, 'valida-responsaveis-temp3.sql'), params_exec=params_exec, conn=conn)
    db.execute_sql(db.get_consulta(params_exec, 'valida-responsaveis-temp4.sql'), params_exec=params_exec, conn=conn)
    db.execute_sql(db.get_consulta(params_exec, 'valida-responsaveis-temp5.sql'), params_exec=params_exec, conn=conn)
    ...
    for x in db.consulta_sql("""select 2 as rest, isnull(chave_dsk1,'') as chs,
                                       ...
                                       where chs is not null""", conn=conn, params_exec=params_exec, index_col='rest'):
        if x['cpf'] is None:
    ...
    for x in db.consulta_sql("""select idMunicipio, 
                                                   ...
                                                   nome_rua,
                                                   nome_bairro
                                              from #tabTempEnderecos""", conn=conn, params_exec=params_exec, index_col='e'):
        print(db.consulta_sql(f"""select * from bethadba.pessoas_enderecos pe where i_pessoas = '{str(x['pessoa'])}'""", conn=conn, params_exec=params_exec))
        if x['p'] != '0':
            if db.consulta_sql(f"""select * from bethadba.pessoas_enderecos pe where i_pessoas = {str(x['pessoa'])}""", conn=conn, params_exec=params_exec).get('p').count() != 0:
                if x['idBairro'] == '':
