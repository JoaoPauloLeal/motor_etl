...
def envia_registro(url, body, params_exec, multipart=False, nome_campo='obras'):
    ...
    try:
        headers = {
            'authorization': f"{params_exec['token']}",
        ...
        if multipart:
            payload = {
                nome_campo: (None, body)
            }
            r = requests.post(url=url, headers=headers, files=payload)
        else:
            headers.update({
...
def buscaFonte(**kwargs):
    ...
    while hasNext:
        offset = pagina * limit
        req = requests.get(url=url, headers=header, params={'limit': limit, 'offset': offset, 'fields': str(fields), 'filter': str(criterio)}) #'sort': 'id asc'
        pagina += 1
        print(req.json())
        if req.ok:
...
def buscaApiTela(**kwargs):
    url = kwargs.get('url')
    token = kwargs.get('token')
    user = kwargs.get('user')

    header = {'content-type': 'application/json', 'Authorization': f'Bearer {token}', 'user-access':user}
...
def buscaServiceLayerSemContent(**kwargs):
    url = kwargs.get('url')
    token = kwargs.get('token')
...
def ExcluirServiceLayerSemJson(**kwargs):
    ...
    return 'Enviado para exclusão: Status - '+str(req.status_code)
...
def ExcluirServiceLayerComJson(**kwargs):
    ...
    print(str(req.status_code)+' - '+str(req.json()))


# Compras

def PUT_service_layer(**kwargs):
    url = kwargs.get('url')
    token = kwargs.get('token')
    data = kwargs.get('data')
    ...
    req = requests.put(url=url, headers=header, data=data)
    vr = None
    # print(req.json())
    if req.ok:
        print('Enviou')
...
def requisicao_service_layer(u_sl, json_rrleq=False):
