...
def checa_existe_registro(params_exec, tipo_registro, chave_dsk1, chave_dsk2, chave_dsk3):
    ...
    if chave_dsk3 != 'null':
        sql += f" AND i_chave_dsk3 = '{chave_dsk3}'"
    # print(sql_teste)
    df = consulta_sql(sql, params_exec, index_col='id')
    print('Checando se existe registro..')
    #dict_dados = df
    if len(df) > 0:
        return True
    return False
...
def get_consulta(params_exec, assunto):
    ...
    try:
        ...
        # Obtém o texto do arquivo assunto.sql_padrao na pasta 'sql_padrao'
...
def get_path(assunto):
    path_padrao = f'packages/{settings.BASE_ORIGEM}/{settings.SISTEMA_ORIGEM}/sql_padrao/'
